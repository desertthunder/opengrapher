...
import type { GenerateOptions, OutputFormat, TemplateName, ThemeOptions } from "./types.ts";
...
function optionalFormat(value: unknown): OutputFormat | undefined {
  ...
  throw new Error(`Config key format must be "png" or "svg".`);
}
...
function optionalBackground(value: unknown) {
  ...
  throw new Error(
    `Config key background must be graph-paper-light, graph-paper-dark, graph-paper-indigo, or graph-paper-warm.`,
  );
}
